...
class ChartQADataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # input_ids
        processed_parse = self.task_prefix + " " + sample['query'] + " " + self.prompt_end_token + " " + sample['label'] + self.processor.tokenizer.eos_token # for plotqa
        ...
        if self.split == "train":
            ...
            return input_tensor, input_ids, labels, query_ids
        else:
            prompt_end_index = torch.nonzero(
                input_ids == self.prompt_end_token_id
            ).sum()  # return prompt end index instead of target output labels
            return input_tensor, input_ids, prompt_end_index, processed_parse, query_ids
