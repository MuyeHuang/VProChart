...
class DonutSwinModel(DonutSwinPreTrainedModel):
    def __init__(self, config, cluster_layer=None, cluster_align=None, add_pooling_layer=True, use_mask_token=False):
        ...
        self.post_init()
        if cluster_layer is not None and cluster_align is not None:
    ...
    def forward(
        ...
        cluster_patch = self.cluster_layer(sequence_output)
        if cluster_patch.dim() == 1:
            cluster_patch = cluster_patch.unsqueeze(0)
        aligned_vit_sequence, alignment_info, pooled_cluster ,cluster= self.cluster_align(sequence_output, cluster_patch, pixel_values, pooled_output, query_ids)

        return DonutSwinModelOutput(
            last_hidden_state=aligned_vit_sequence,
